...
const PublicView: React.FC<PublicViewProps> = ({ files, onReadPdf }) => {
  ...
  return (
    <div>
      ...
      {files.length > 0 && filteredFiles.length === 0 ? (
          ...
          <p className="text-center text-slate-400">Nenhum catálogo disponível no momento.</p>
        ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredFiles.map(file => (
            <PdfCard key={file.id} file={file} isAdmin={false} onRead={onReadPdf} />
          ))}
        </div>
      )}
    </div>
